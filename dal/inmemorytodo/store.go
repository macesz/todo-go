...
func NewInMemoryStore() *InMemoryStore {
	return &InMemoryStore{nextID: 1, data: make(map[int]domain.Todo)} // make() initializes the map
}
...
func (s *InMemoryStore) Create(title string) (domain.Todo, error) {
...
func (s *InMemoryStore) List() []domain.Todo {
...
func (s *InMemoryStore) Get(id int) (domain.Todo, bool) {
	s.mu.RLock()         // Read lock (like synchronized block in Java)
	defer s.mu.RUnlock() // defer ensures unlock happens (like finally in Java)
...
func (s *InMemoryStore) Update(id int, title string, done bool) (domain.Todo, error) {
